...
func TestUint8SliceCompat(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Old write -> new read
			readBack2, err := readSlice[uint8](bufio.NewReader(bytes.NewReader(oldBuf.Bytes())), binary.BigEndian)
			require.NoError(t, err)
...
func TestUint16SliceCompat(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Old write -> new read
			readBack2, err := readSlice[uint16](bufio.NewReader(bytes.NewReader(oldBuf.Bytes())), binary.BigEndian)
			require.NoError(t, err)
