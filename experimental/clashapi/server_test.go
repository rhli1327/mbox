...
func TestTrafficHistoryAPIMountContract(t *testing.T) {
	history, availableFrom := newClashAPITestHistory(t)
	router := chi.NewRouter()
	router.Group(func(router chi.Router) {
		router.Use(authentication("traffic-secret"))
		mountTrafficHistoryAPI(router, history)
	})
	...
	assertClashJSONString(t, unauthorizedBody, "message", "Unauthorized")

	capabilities := performClashTrafficRequest(
		t,
		router,
		http.MethodGet,
	...
	for _, required := range []string{
		...
		if !slicesContains(capabilitiesBody.Dimensions, required) {
			t.Fatalf("capabilities omitted dimension %q: %#v", required, capabilitiesBody.Dimensions)
		}
	}
	...
	query := performClashTrafficRequest(
		t,
		router,
		http.MethodPost,
	...
	assertClashMountedQuery(t, query.Body.Bytes(), false)

	targetQuery := performClashTrafficRequest(
		t,
		router,
		http.MethodPost,
		"/mbox/v2/traffic/query",
	...
	assertClashMountedQuery(t, targetQuery.Body.Bytes(), true)

	invalid := performClashTrafficRequest(
		t,
		router,
		http.MethodPost,
		"/mbox/v2/traffic/query",
	...
	assertClashJSONString(t, invalidBody, "error", "page must be at least 1")
}

func newClashAPITestHistory(t *testing.T) (*trafficcontrol.History, time.Time) {
	t.Helper()
	path := filepath.Join(t.TempDir(), "traffic.db")
	availableFrom := time.Now().UTC().Truncate(trafficcontrol.HistoryBucketInterval)
	seedClashAPITestAvailability(t, path, availableFrom)
...
func seedClashAPITestAvailability(t *testing.T, path string, availableFrom time.Time) {
	t.Helper()
...
func performClashTrafficRequest(
	t *testing.T,
...
func assertClashMountedQuery(t *testing.T, content []byte, target bool) {
	t.Helper()
	var body struct {
	...
	if target {
		...
		assertClashJSONString(t, body.Rows[0], "destination", "api.example")
		assertClashJSONString(t, body.Rows[0], "destination_type", trafficcontrol.HistoryDestinationTypeDomain)
		assertClashJSONString(t, body.Rows[0], "destination_domain", "api.example")
	} else {
		...
		assertClashJSONString(t, body.Rows[0], "route_tag", "Proxy")
		assertClashStringArray(t, body.Rows[0], "group_path", []string{"Proxy", "Auto"})
		assertClashJSONString(t, body.Rows[0], "outbound_group", "Auto")
		assertClashJSONString(t, body.Rows[0], "actual_outbound_tag", "node-a")
		assertClashJSONString(t, body.Rows[0], "actual_outbound_type", "vmess")
		assertClashJSONString(t, body.Rows[0], "network", N.NetworkTCP)
		assertClashNonEmptyJSONString(t, body.Rows[0], "config_revision")
	}
	for _, fields := range []map[string]json.RawMessage{body.Totals, body.Rows[0]} {
		assertClashJSONString(t, fields, "uplink_bytes", "9007199254740993")
		assertClashJSONString(t, fields, "downlink_bytes", "9007199254740995")
		assertClashJSONString(t, fields, "connections", "1")
	}
}

func assertClashJSONString(t *testing.T, object map[string]json.RawMessage, key string, expected string) {
	t.Helper()
...
func assertClashNonEmptyJSONString(t *testing.T, object map[string]json.RawMessage, key string) {
	t.Helper()
	content, loaded := object[key]
	if !loaded {
		t.Fatalf("missing JSON field %q in %#v", key, object)
...
func assertClashStringArray(t *testing.T, object map[string]json.RawMessage, key string, expected []string) {
	t.Helper()
	content, loaded := object[key]
	if !loaded {
		t.Fatalf("missing JSON field %q in %#v", key, object)
